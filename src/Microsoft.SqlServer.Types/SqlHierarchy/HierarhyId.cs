...
namespace Microsoft.SqlServer.Types.SqlHierarchy
{
    ...
    internal struct HierarchyId : IComparable
    {
        ...
        public HierarchyId GetDescendant(HierarchyId? child1, HierarchyId? child2)
        {
            ...
            {
                child1LastNode = child1LastNode.Concat(new[] { 0L }).ToArray();
            }
            ...
            hierarchyStr = PathSeparator + string.Join(PathSeparator, GetNodes().Select(LongArrayToString)) + PathSeparator + LongArrayToString(child1LastNode) + PathSeparator;
            return new HierarchyId(hierarchyStr);
        }
